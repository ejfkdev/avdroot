...
`,
  ...
  avdroot root --emulator-args "-no-window -gpu swiftshader_indirect"`,
	...
	"root.env_setup":              "Preparing Magisk's environment",
	"root.env_ready":              "the app has already unpacked Magisk into /data/adb",
	"root.env_missing":            "the app has not unpacked Magisk yet, so the su on PATH is the emulator's own",
	"root.env_opening":            "opening the Magisk app so it can finish installing itself",
	"root.env_answered":           "the app asked to set itself up; pressed \"{0}\"",
	"root.env_no_dialog":          "the Magisk app did not ask to set itself up",
	"root.env_no_dialog_hint":     "if su still fails, open the Magisk app once and run \"avdroot root\" again",
	"root.env_rebooting":          "the app restarts the device to finish; waiting for it to come back",
	"root.env_rebooted":           "the device restarted with Magisk's environment in place",
	...
	"root.shell_root_hint":        "Magisk's su could not be tested while adbd is root; run \"adb unroot\" to check it properly",
	"root.dropping_adbd_root":     "giving up adbd's root first, so that the check tests Magisk and not the emulator",
