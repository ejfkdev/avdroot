...
`,
  ...
  avdroot root --emulator-args "-no-window -gpu swiftshader_indirect"`,
	...
	"root.env_ready":              "Magisk {0} is already unpacked under /data/adb",
	"root.env_missing":            "Magisk's files are not unpacked yet; writing them for {0} now",
	"root.env_other_version":      "Magisk's files belong to another version; replacing them with {0}",
	"root.env_installed":          "{0} files written for Magisk {1}, before the restart that will use them",
	"root.env_unknown":            "could not tell whether Magisk's files are in place: {0}",
