...
`,
  ...
  avdroot root --emulator-args "-no-window -gpu swiftshader_indirect"`,
	...
	"root.env_ready":              "App 已经把 Magisk 解压到 /data/adb",
	"root.env_missing":            "App 还没解压 Magisk，所以 PATH 里的 su 是模拟器自带的那一个",
	"root.env_opening":            "启动 Magisk App，让它完成自我安装",
	"root.env_answered":           "App 提示需要额外设置，已点击「{0}」",
	"root.env_no_dialog":          "Magisk App 没有提示需要额外设置",
	"root.env_no_dialog_hint":     "如果 su 仍然失败，手动打开一次 Magisk App，再运行 \"avdroot root\"",
	"root.env_rebooting":          "App 会重启设备来完成设置，正在等待它回来",
	"root.env_rebooted":           "设备已重启，Magisk 运行环境就绪",
