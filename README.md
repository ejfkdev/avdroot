...
| `--no-install` | `root` | Skip installing the Magisk app; rooting does not depend on it |
...
and belongs in `/data/adb/magisk`, and `magiskd` decides at startup whether that
directory is complete: until it is, `su` is simply not put on `PATH`, and the
`su` that is there is the emulator's own, which reads `-c` as a uid and fails
with `su: invalid uid/gid '-c'`. Nothing looks wrong in the meantime — Magisk
reports its version and accepts su-policy writes the whole time — so the symptom
is a tool that declares Magisk healthy and then cannot use it.

Magisk's own check is the `env_check` shell function in
[`scripts/app_functions.sh`](https://github.com/topjohnwu/Magisk/blob/master/scripts/app_functions.sh),
and its fix, `fix_env`, is a copy into that directory. Neither is a `magisk`
subcommand, and neither touches the boot image. `avdroot` reproduces both, which
is why the app, its setup dialog and a second reboot are all unnecessary: the
files go in before the restart that was happening anyway, so the first boot with
a patched ramdisk already has them.
...
internal/adb/           adb wrapper, su policy, preinit detection, Magisk's environment
...
`go test ./...` passes on a machine with no Android SDK — 99 tests run, 14 skip.
